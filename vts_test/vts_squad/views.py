...
def job_submit(request):
    if request.method == "GET":
        ...
        try:
            ...
            res = submit_to_squad.delay(job.id, verify_url, \
...
def job_info_detail(request, jid):
    ...
    if not job.lava_job:
        ...
        log_file_path = os.path.join(DOWNLOAD_DIR, job.jenkins_build_num.strip("['").strip("']"), log_file_name)
    ...
    server = xmlrpclib.ServerProxy("http://%s:%s@%s/RPC2" % ('apuser',
